...
test.describe.serial('On This Day', () => {
  ...
  test('shows an empty personal state after moving to a date with no matches', async ({ page }) => {
    await installWikipediaMock(page);
    await gotoHome(page);
  ...
  test('switches to the history tab and renders mocked wikipedia events', async ({ page }) => {
    ...
    await expect(page.locator('#history-container')).toContainText('Events: Historical milestone');
    await expect(page.locator('#history-container')).toContainText('Births: Another historical event');
    await expect(page.locator('#history-container a[href="https://en.wikipedia.org/wiki/On_This_Day"]')).toBeVisible();
    await expect(page.getByRole('heading', { name: 'On This Day...' })).toBeVisible();
  ...
  test('refetches wikipedia data when the selected date changes on the history tab', async ({ page }) => {
    const wikiRequests = await installWikipediaMock(page);
    await gotoHome(page);
    ...
    await expect(page.getByLabel('Reset to current date')).toContainText(tomorrowSubtitle);
    await expect(page.locator('#history-container')).toContainText('Events: Next day historical event');
    await expect.poll(() => wikiRequests.map((request) => request.date)).toEqual([
      todayIso,
      tomorrowIso,
      tomorrowIso,
    ]);
  ...
  test('changes the date subtitle with the previous, next, and reset controls', async ({ page }) => {
    const wikiRequests = await installWikipediaMock(page);
    await gotoHome(page);
